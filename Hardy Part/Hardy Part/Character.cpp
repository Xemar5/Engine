...
std::shared_ptr<Entity> Character::Add(unsigned stt, std::string name, int x, int y, unsigned layer, double scale)
{
	if (name.size() == 0)
	{
	...
	if (name == "Nerk")
	{
		auto ent = State::Built[stt]->Add_Entity<Character>(layer);
