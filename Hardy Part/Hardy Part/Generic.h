...
public:
	...
	static std::shared_ptr<Texture> Load(std::vector<std::vector<int>> map, std::shared_ptr<Entity> ent, std::string path, unsigned width, unsigned height, float starting_point_x = 0, float starting_point_y = 0, int frame_width = 0, int frame_height = 0);
	...
	static int Which_Tile(std::shared_ptr<Entity> ent, int x, int y);
	//unsigned Which_Tile(int x, int y);

	//*** Returns the visible SDL_Texture created by this Tileset class
	//*** Created when Change function is called
